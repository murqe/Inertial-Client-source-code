...
public class RenderIllusionIllager extends RenderLiving {
      ...
      public void doRender(EntityMob entity, double x, double y, double z, float entityYaw, float partialTicks) {
            if (entity.isInvisible()) {
                  ...
                  for(int i = 0; i < avec3d.length; ++i) {
                        super.doRender((EntityLiving)entity, x + avec3d[i].xCoord + (double)MathHelper.cos((float)i + f * 0.5F) * 0.025D, y + avec3d[i].yCoord + (double)MathHelper.cos((float)i + f * 0.75F) * 0.0125D, z + avec3d[i].zCoord + (double)MathHelper.cos((float)i + f * 0.7F) * 0.025D, entityYaw, partialTicks);
                  }
            } else {
