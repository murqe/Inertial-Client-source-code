                  ...
                  public float apply(ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn) {
                        if (entityIn == null && !stack.isOnItemFrame()) {
                              ...
                              if (worldIn.provider.isSurfaceWorld()) {
                                    double d1 = flag ? (double)((Entity)entity).rotationYaw : this.getFrameRotation((EntityItemFrame)entity);
                                    d1 = MathHelper.func_191273_b(d1 / 360.0D, 1.0D);
                                    double d2 = this.getSpawnToAngle(worldIn, (Entity)entity) / 6.283185307179586D;
                                    d0 = 0.5D - (d1 - 0.25D - d2);
                              } else {
                                    d0 = Math.random();
                              }
