...
public class Template {
      ...
      private void takeEntitiesFromWorld(World worldIn, BlockPos startPos, BlockPos endPos) {
            List list = worldIn.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(startPos, endPos), new Predicate() {
            ...
            for(Iterator var5 = list.iterator(); var5.hasNext(); this.entities.add(new Template.EntityInfo(vec3d, blockpos, nbttagcompound))) {
                  ...
                  vec3d = new Vec3d(entity.posX - (double)startPos.getX(), entity.posY - (double)startPos.getY(), entity.posZ - (double)startPos.getZ());
      ...
      public Map getDataBlocks(BlockPos pos, PlacementSettings placementIn) {
            ...
            if ((!this.blocks.isEmpty() || !p_189960_4_.getIgnoreEntities() && !this.entities.isEmpty()) && this.size.getX() >= 1 && this.size.getY() >= 1 && this.size.getZ() >= 1) {
                  ...
                  while(true) {
                              ...
                              do {
                                    do {
                                          do {
                                                ...
                                                if (!var8.hasNext()) {
                                                      ...
                                                      while(true) {
                                                            do {
                                                                  ...
                                                                  blockpos = transformedBlockPos(p_189960_4_, template$blockinfo2.pos).add(p_189960_2_);
                                                            } while(structureboundingbox != null && !structureboundingbox.isVecInside(blockpos));

                                                            p_189960_1_.notifyNeighborsRespectDebug(blockpos, template$blockinfo2.blockState.getBlock(), false);
                                                            if (template$blockinfo2.tileentityData != null) {
                                                                  TileEntity tileentity1 = p_189960_1_.getTileEntity(blockpos);
                                                                  if (tileentity1 != null) {
                                                ...
                                                template$blockinfo2 = (Template.BlockInfo)var8.next();
                                                blockpos = transformedBlockPos(p_189960_4_, template$blockinfo2.pos).add(p_189960_2_);
                                                template$blockinfo1 = p_189960_3_ != null ? p_189960_3_.processBlock(p_189960_1_, blockpos, template$blockinfo2) : template$blockinfo2;
                                          } while(template$blockinfo1 == null);

                                          block1 = template$blockinfo1.blockState.getBlock();
                                    } while(block != null && block == block1);
                              } while(p_189960_4_.getIgnoreStructureBlock() && block1 == Blocks.STRUCTURE_BLOCK);
                        } while(structureboundingbox != null && !structureboundingbox.isVecInside(blockpos));
                        ...
                        if (template$blockinfo1.tileentityData != null) {
                              tileentity2 = p_189960_1_.getTileEntity(blockpos);
                              if (tileentity2 != null) {
