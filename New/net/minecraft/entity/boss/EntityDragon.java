...
public class EntityDragon extends EntityLiving implements IEntityMultiPart, IMob {
     ...
     public static final DataParameter PHASE;
     ...
     public void onLivingUpdate() {
          ...
          if (this.world.isRemote) {
               ...
               if (!this.isSilent()) {
                    f11 = MathHelper.cos(this.animTime * 6.2831855F);
          ...
          float f14;
          if (this.getHealth() <= 0.0F) {
               f11 = (this.rand.nextFloat() - 0.5F) * 8.0F;
               f1 = (this.rand.nextFloat() - 0.5F) * 4.0F;
               f14 = (this.rand.nextFloat() - 0.5F) * 8.0F;
               this.world.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX + (double)f11, this.posY + 2.0D + (double)f1, this.posZ + (double)f14, 0.0D, 0.0D, 0.0D);
          } else {
               this.updateDragonEnderCrystal();
               f11 = 0.2F / (MathHelper.sqrt(this.motionX * this.motionX + this.motionZ * this.motionZ) * 10.0F + 1.0F);
               ...
               if (this.isAIDisabled()) {
                    ...
                    if (this.world.isRemote) {
                         if (this.newPosRotationIncrements > 0) {
                              ...
                              d6 = this.posY + (this.interpTargetY - this.posY) / (double)this.newPosRotationIncrements;
                              d7 = this.posZ + (this.interpTargetZ - this.posZ) / (double)this.newPosRotationIncrements;
                              d8 = MathHelper.wrapDegrees(this.interpTargetYaw - (double)this.rotationYaw);
                         ...
                         if (vec3d != null) {
                              d6 = vec3d.xCoord - this.posX;
                              d7 = vec3d.yCoord - this.posY;
                              d8 = vec3d.zCoord - this.posZ;
                              double d3 = d6 * d6 + d7 * d7 + d8 * d8;
                              f20 = iphase.getMaxRiseOrFall();
                              d7 = MathHelper.clamp(d7 / (double)MathHelper.sqrt(d6 * d6 + d8 * d8), (double)(-f20), (double)f20);
                              ...
                              f6 = Math.max(((float)vec3d2.dotProduct(vec3d1) + 0.5F) / 1.5F, 0.0F);
                              this.randomYawVelocity *= 0.8F;
                              this.randomYawVelocity = (float)((double)this.randomYawVelocity + d4 * (double)iphase.getYawFactor());
                              this.rotationYaw += this.randomYawVelocity * 0.1F;
                              f22 = (float)(2.0D / (d3 + 1.0D));
                              f23 = 0.06F;
                              this.func_191958_b(0.0F, 0.0F, -1.0F, 0.06F * (f6 * f22 + (1.0F - f22)));
                              if (this.slowed) {
                    ...
                    f14 = (float)(this.getMovementOffsets(5, 1.0F)[1] - this.getMovementOffsets(10, 1.0F)[1]) * 10.0F * 0.017453292F;
                    ...
                    for(k = 0; k < this.dragonPartArray.length; ++k) {
                         this.dragonPartArray[k].prevPosX = avec3d[k].xCoord;
                         this.dragonPartArray[k].prevPosY = avec3d[k].yCoord;
                         this.dragonPartArray[k].prevPosZ = avec3d[k].zCoord;
                    }
               }
          }
     ...
     private void updateDragonEnderCrystal() {
          ...
          if (this.rand.nextInt(10) == 0) {
               ...
               while(var5.hasNext()) {
                    EntityEnderCrystal entityendercrystal1 = (EntityEnderCrystal)var5.next();
     ...
     public int initPathPoints() {
          if (this.pathPoints[0] == null) {
               for(int i = 0; i < 24; ++i) {
                    ...
                    lvt_3_1_ = Math.max(this.world.getSeaLevel() + 10, this.world.getTopSolidOrLiquidBlock(new BlockPos(l, 0, i1)).getY() + j);
                    this.pathPoints[i] = new PathPoint(l, lvt_3_1_, i1);
               }
     ...
     public Path findPath(int startIdx, int finishIdx, @Nullable PathPoint andThen) {
          ...
          while(!this.pathFindQueue.isPathEmpty()) {
               ...
               int i1;
               for(i1 = 0; i1 < 24; ++i1) {
                    if (this.pathPoints[i1] == pathpoint2) {
                         k = i1;
                         break;
                    }
               }
     ...
     public Vec3d getHeadLookVec(float p_184665_1_) {
          ...
          if (phaselist != PhaseList.LANDING && phaselist != PhaseList.TAKEOFF) {
               ...
               BlockPos blockpos = this.world.getTopSolidOrLiquidBlock(WorldGenEndPodium.END_PODIUM_LOCATION);
               f5 = Math.max(MathHelper.sqrt(this.getDistanceSqToCenter(blockpos)) / 4.0F, 1.0F);
               float f1 = 6.0F / f5;
     ...
     public void onCrystalDestroyed(EntityEnderCrystal crystal, BlockPos pos, DamageSource dmgSrc) {
          ...
          if (crystal == this.healingEnderCrystal) {
               this.attackEntityFromPart(this.dragonPartHead, DamageSource.causeExplosionDamage((EntityLivingBase)entityplayer), 10.0F);
          }
