...
public abstract class LayerArmorBase implements LayerRenderer {
     ...
     public void doRenderLayer(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
          if (!Main.instance.featureDirector.getFeatureByClass(NoRender.class).isToggled() || !NoRender.noArmor.getBoolValue()) {
     ...
     private void renderArmorLayer(EntityLivingBase entityLivingBaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale, EntityEquipmentSlot slotIn) {
          if (!Main.instance.featureDirector.getFeatureByClass(CustomModel.class).isToggled() || !(entityLivingBaseIn instanceof EntityPlayerSP)) {
               ItemStack itemstack = entityLivingBaseIn.getItemStackFromSlot(slotIn);
               if (itemstack.getItem() instanceof ItemArmor) {
                    ItemArmor itemarmor = (ItemArmor)itemstack.getItem();
                    if (itemarmor.getEquipmentSlot() == slotIn) {
                         ModelBase t = this.getModelFromSlot(slotIn);
          ...
          if (!Config.isShaders() || !Shaders.isShadowPass) {
               ...
               for(int i = 0; i < 2; ++i) {
                    ...
                    if (Main.instance.featureDirector.getFeatureByClass(EnchantmentColor.class).isToggled()) {
                         String var12 = EnchantmentColor.colorMode.currentMode;
                         byte var13 = -1;
                         switch(var12.hashCode()) {
                         case -1656737386:
                              if (var12.equals("Rainbow")) {
                                   var13 = 0;
                              }
                              break;
                         case 2021122027:
                              if (var12.equals("Client")) {
                                   var13 = 2;
                              }
                              break;
                         case 2029746065:
                              if (var12.equals("Custom")) {
                                   var13 = 1;
                              }
                         }
                         ...
                         switch(var13) {
                         case 0:
                              Color rainbow = DrawHelper.rainbow((int)f, 1.0F, 1.0F);
                              GlStateManager.color((float)rainbow.getRed(), (float)rainbow.getGreen(), (float)rainbow.getBlue(), (float)rainbow.getAlpha());
                              break;
                         case 1:
                              Color colorCustom = new Color(EnchantmentColor.customColor.getColorValue());
                              GlStateManager.color((float)colorCustom.getRed(), (float)colorCustom.getGreen(), (float)colorCustom.getBlue(), (float)colorCustom.getAlpha());
                              break;
                         case 2:
                              GlStateManager.color((float)ClientHelper.getClientColor().getRed(), (float)ClientHelper.getClientColor().getGreen(), (float)ClientHelper.getClientColor().getBlue(), (float)ClientHelper.getClientColor().getAlpha());
                         }
                    }
     ...
     protected ModelBase getArmorModelHook(EntityLivingBase p_getArmorModelHook_1_, ItemStack p_getArmorModelHook_2_, EntityEquipmentSlot p_getArmorModelHook_3_, ModelBase p_getArmorModelHook_4_) {
