          ...
          if (p_192841_9_ < 0) {
               for(int j = 0; j < p_192841_1_.signText.length; ++j) {
                    if (p_192841_1_.signText[j] != null) {
                         ...
                         List list = GuiUtilRenderComponents.splitText(itextcomponent, 90, fontrenderer, false, true);
                         String s = list != null && !list.isEmpty() ? ((ITextComponent)list.get(0)).getFormattedText() : "";
                         if (j == p_192841_1_.lineBeingEdited) {
                              s = "> " + s + " <";
                              fontrenderer.drawString(s, (float)(-fontrenderer.getStringWidth(s) / 2), (float)(j * 10 - p_192841_1_.signText.length * 5), i);
                         } else {
                              fontrenderer.drawString(s, (float)(-fontrenderer.getStringWidth(s) / 2), (float)(j * 10 - p_192841_1_.signText.length * 5), i);
                         }
                    }
