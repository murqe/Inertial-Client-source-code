...
public class GuiIngame extends Gui {
     ...
     public void renderGameOverlay(float partialTicks) {
          ...
          if (this.mc.player.getSleepTimer() > 0) {
               ...
               k1 = this.mc.player.getSleepTimer();
               f3 = (float)k1 / 100.0F;
               if (f3 > 1.0F) {
          ...
          if (this.recordPlayingUpFor > 0) {
               ...
               if (i2 > 8) {
                    ...
                    fontrenderer.drawString(this.recordPlaying, (float)(-fontrenderer.getStringWidth(this.recordPlaying) / 2), -4.0F, j2 + (i2 << 24 & -16777216));
                    GlStateManager.disableBlend();
                    ...
                    if (flag && potioneffect.doesShowParticles()) {
                         ...
                         if (Reflector.ForgePotion_renderHUDEffect.exists()) {
                              if (potion.hasStatusIcon()) {
                                   this.drawTexturedModalRect(k + 4, -l + sr.getScaledHeight() - 20, i1 % 8 * 18, 198 + i1 / 8 * 18, 18, 18);
                              }

                              Reflector.call(potion, Reflector.ForgePotion_renderHUDEffect, new Object[]{k, l, potioneffect, this.mc, f});
                         } else {
                              this.drawTexturedModalRect(k + 4, -l + sr.getScaledHeight() - 20, i1 % 8 * 18, 198 + i1 / 8 * 18, 18, 18);
                         }
                    }
     ...
     protected void renderHotbar(ScaledResolution sr, float partialTicks) {
          if (!(this.mc.currentScreen instanceof GuiChat)) {
               ...
               if (Main.instance.featureDirector.getFeatureByClass(Hotbar.class).isToggled()) {
                    drawRect(0.0D, (double)(sr.getScaledHeight() - 23) - Feature.deltaTime(), (double)sr.getScaledWidth(), (double)sr.getScaledHeight(), -1728053248);
                    double target = (double)(sr.getScaledWidth() / 2 - 91 + this.mc.player.inventory.currentItem * 20);
                    double var10000 = target - this.last;
                    Minecraft var10001 = this.mc;
                    double delta = var10000 / (double)Math.max((float)Minecraft.getDebugFPS(), 5.0F) * 15.0D;
                    if (Math.abs(delta) > Math.abs(target - this.last)) {
                         this.last = target;
                    } else {
                         this.last += delta;
                    }

                    DrawHelper.drawRect((double)((float)this.last), (double)(sr.getScaledHeight() - 23) - Feature.deltaTime(), (double)((float)this.last + 182.0F - (float)(this.mc.player.inventory.currentItem * 20) - (float)(20 * (8 - this.mc.player.inventory.currentItem))), (double)sr.getScaledHeight() - Feature.deltaTime(), ClientHelper.getClientColor().getRGB());
                    GlStateManager.disableBlend();
                    ...
                    enumhandside = entityplayer.getPrimaryHand().opposite();
                    i = sr.getScaledWidth() / 2;
                    f = this.zLevel;
                    int j = true;
                    int k = true;
                    this.zLevel = -90.0F;
                    this.drawTexturedModalRect((float)(i - 91), (float)((double)(sr.getScaledHeight() - 22) - Feature.deltaTime()), 0, 0, 182, 22);
                    this.drawTexturedModalRect((float)(i - 91 - 1 + entityplayer.inventory.currentItem * 20), (float)((double)(sr.getScaledHeight() - 22 - 1) - Feature.deltaTime()), 0, 22, 24, 22);
               }
               ...
               if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
                    ...
                    f = this.zLevel;
                    this.zLevel = -90.0F;
                    if (Main.instance.featureDirector.getFeatureByClass(Hotbar.class).isToggled() && !itemstack.func_190926_b()) {
                         if (enumhandside == EnumHandSide.LEFT) {
                              this.drawTexturedModalRect((float)(i - 91 - 29), (float)((double)(sr.getScaledHeight() - 23) - Feature.deltaTime()), 24, 22, 29, 24);
                         } else {
                              this.drawTexturedModalRect((float)(i + 91), (float)((double)(sr.getScaledHeight() - 23) - Feature.deltaTime()), 53, 22, 29, 24);
                         }
                    }
                    ...
                    for(l1 = 0; l1 < 9; ++l1) {
                         ...
                         this.renderHotbarItem(i2, (int)((double)j2 - Feature.deltaTime()), partialTicks, entityplayer, (ItemStack)entityplayer.inventory.mainInventory.get(l1));
                    }

                    if (!itemstack.func_190926_b()) {
                         CustomItems.setRenderOffHand(true);
                         l1 = (int)((double)(sr.getScaledHeight() - 16 - 3) - Feature.deltaTime());
                         if (enumhandside == EnumHandSide.LEFT) {
     ...
     private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
          ...
          while(var12.hasNext()) {
               ...
               this.getFontRenderer().drawString(s1, (float)l1, (float)k, 553648127);
               this.getFontRenderer().drawString(s2, (float)(l - this.getFontRenderer().getStringWidth(s2)), (float)k, 553648127);
               if (j == collection.size()) {
                    ...
                    this.getFontRenderer().drawString(s3, (float)(l1 + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2), (float)(k - this.getFontRenderer().FONT_HEIGHT), 553648127);
               }
          }
