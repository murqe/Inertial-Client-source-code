          ...
          if (position.getY() >= 1 && position.getY() + i + 1 < 256) {
               ...
               for(int j = position.getY(); j <= position.getY() + 1 + i; ++j) {
                    ...
                    for(j3 = position.getX() - k; j3 <= position.getX() + k && flag; ++j3) {
                         for(k3 = position.getZ() - k; k3 <= position.getZ() + k && flag; ++k3) {
                              if (j >= 0 && j < 256) {
                                   Material material = worldIn.getBlockState(blockpos$mutableblockpos.setPos(j3, j, k3)).getMaterial();
               ...
               if (!flag) {
                    ...
                    if (block1 != Blocks.DIRT && block1 != Blocks.GRASS && block1 != Blocks.MYCELIUM) {
                         ...
                         for(l2 = 0; l2 < i; ++l2) {
                              IBlockState iblockstate = worldIn.getBlockState(position.up(l2));
                              if (!iblockstate.isFullBlock()) {
                                   this.setBlockAndNotifyAdequately(worldIn, position.up(l2), block.getDefaultState().withProperty(BlockHugeMushroom.VARIANT, BlockHugeMushroom.EnumType.STEM));
                              }
                         }
