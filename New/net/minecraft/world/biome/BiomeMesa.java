...
public class BiomeMesa extends Biome {
     ...
     public void genTerrainBlocks(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal) {
          ...
          if (this.brycePillars) {
               k1 = (x & -16) + (z & 15);
               l1 = (z & -16) + (x & 15);
               double d0 = Math.min(Math.abs(noiseVal), this.pillarNoise.getValue((double)k1 * 0.25D, (double)l1 * 0.25D));
               if (d0 > 0.0D) {
                    double d1 = 0.001953125D;
                    double d2 = Math.abs(this.pillarRoofNoise.getValue((double)k1 * 0.001953125D, (double)l1 * 0.001953125D));
                    d4 = d0 * d0 * 2.5D;
          ...
          boolean flag = Math.cos(noiseVal / 3.0D * 3.141592653589793D) > 0.0D;
          int l = -1;
          boolean flag1 = false;
          int i1 = 0;
