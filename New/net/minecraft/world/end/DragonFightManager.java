...
public class DragonFightManager {
     ...
     private BlockPattern.PatternHelper findExitPortal() {
          ...
          for(i = -8; i <= 8; ++i) {
               for(l = -8; l <= 8; ++l) {
                    Chunk chunk = this.world.getChunkFromChunkCoords(i, l);
                    Iterator var4 = chunk.getTileEntityMap().values().iterator();
     ...
     private void updateplayers() {
          ...
          Iterator var2 = this.world.getPlayers(EntityPlayerMP.class, VALID_PLAYER).iterator();
          ...
          Set set1 = Sets.newHashSet(this.bossInfo.getPlayers());
          set1.removeAll(set);
          Iterator var6 = set1.iterator();

          while(var6.hasNext()) {
               EntityPlayerMP entityplayermp1 = (EntityPlayerMP)var6.next();
     ...
     private void findAliveCrystals() {
          ...
          WorldGenSpikes.EndSpike[] var1 = BiomeEndDecorator.getSpikesForWorld(this.world);
     ...
     public void respawnDragon() {
          if (this.dragonKilled && this.respawnState == null) {
               ...
               while(var4.hasNext()) {
                    ...
                    List list = this.world.getEntitiesWithinAABB(EntityEnderCrystal.class, new AxisAlignedBB(blockpos1.offset(enumfacing, 2)));
                    if (list.isEmpty()) {
                         return;
                    }
     ...
     private void respawnDragon(List crystalsIn) {
          if (this.dragonKilled && this.respawnState == null) {
     ...
     public void resetSpikeCrystals() {
          ...
          for(int var3 = 0; var3 < var2; ++var3) {
               WorldGenSpikes.EndSpike worldgenspikes$endspike = var1[var3];
               Iterator var5 = this.world.getEntitiesWithinAABB(EntityEnderCrystal.class, worldgenspikes$endspike.getTopBoundingBox()).iterator();
     ...
     static {
          VALID_PLAYER = Predicates.and(EntitySelectors.IS_ALIVE, EntitySelectors.withinRange(0.0D, 128.0D, 0.0D, 192.0D));
     }
}
