...
public class FlyingNodeProcessor extends WalkNodeProcessor {
     ...
     public int findPathOptions(PathPoint[] pathOptions, PathPoint currentPoint, PathPoint targetPoint, float maxDistance) {
          ...
          if (flag && flag3) {
               pathpoint17 = this.openPoint(currentPoint.xCoord - 1, currentPoint.yCoord, currentPoint.zCoord - 1);
          ...
          if (flag && flag4) {
               pathpoint17 = this.openPoint(currentPoint.xCoord, currentPoint.yCoord + 1, currentPoint.zCoord - 1);
               if (pathpoint17 != null && !pathpoint17.visited && pathpoint17.distanceTo(targetPoint) < maxDistance) {
                    pathOptions[i++] = pathpoint17;
               }
          }
          ...
          if (flag2 && flag4) {
               pathpoint17 = this.openPoint(currentPoint.xCoord + 1, currentPoint.yCoord + 1, currentPoint.zCoord);
               if (pathpoint17 != null && !pathpoint17.visited && pathpoint17.distanceTo(targetPoint) < maxDistance) {
                    pathOptions[i++] = pathpoint17;
               }
          }
     ...
     public PathNodeType getPathNodeType(IBlockAccess blockaccessIn, int x, int y, int z, EntityLiving entitylivingIn, int xSize, int ySize, int zSize, boolean canBreakDoorsIn, boolean canEnterDoorsIn) {
          ...
          if (enumset.contains(PathNodeType.FENCE)) {
               ...
               while(var15.hasNext()) {
                    PathNodeType pathnodetype2 = (PathNodeType)var15.next();
