...
public class GuiOverlayDebug extends Gui {
      ...
      protected void renderDebugInfoLeft() {
            ...
            for(int i = 0; i < list.size(); ++i) {
                  ...
                  if (!Strings.isNullOrEmpty(s)) {
                        ...
                        int l = true;
                        int i1 = 2 + j * i;
                        drawRect(1.0D, (double)(i1 - 1), (double)(2 + k + 1), (double)(i1 + j - 1), -1873784752);
      ...
      protected void renderDebugInfoRight(ScaledResolution scaledRes) {
            ...
            for(int i = 0; i < list.size(); ++i) {
                  ...
                  if (!Strings.isNullOrEmpty(s)) {
                        ...
                        int i1 = 2 + j * i;
                        drawRect((double)(l - 1), (double)(i1 - 1), (double)(l + k + 1), (double)(i1 + j - 1), -1873784752);
                        ...
                        if (this.mc.world.isBlockLoaded(blockpos) && blockpos.getY() >= 0 && blockpos.getY() < 256) {
                              if (!chunk.isEmpty()) {
                                    ...
                                    if (this.mc.isIntegratedServerRunning() && this.mc.getIntegratedServer() != null) {
                                          PlayerList var10000 = this.mc.getIntegratedServer().getPlayerList();
                                          Minecraft var10001 = this.mc;
                                          EntityPlayerMP entityplayermp = var10000.getPlayerByUUID(Minecraft.player.getUniqueID());
      ...
      protected List getDebugInfoRight() {
            ...
            long l = j - k;
            List list = Lists.newArrayList(new String[]{String.format("Java: %s %dbit", System.getProperty("java.version"), this.mc.isJava64bit() ? 64 : 32), String.format("Mem: % 2d%% %03d/%03dMB", l * 100L / i, bytesToMb(l), bytesToMb(i)), String.format("Allocated: % 2d%% %03dMB", j * 100L / i, bytesToMb(j)), "", String.format("CPU: %s", OpenGlHelper.getCpu()), "", String.format("Display: %dx%d (%s)", Display.getWidth(), Display.getHeight(), GlStateManager.glGetString(7936)), GlStateManager.glGetString(7937), GlStateManager.glGetString(7938)});
