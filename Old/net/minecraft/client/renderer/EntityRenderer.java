...
public class EntityRenderer implements IResourceManagerReloadListener {
      ...
      private void hurtCameraEffect(float partialTicks) {
            if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
                  ...
                  if (Main.settingsManager.getSettingByName("HurtCam").getValBoolean() && Main.moduleManager.getModule(NoRender.class).isToggled()) {
      ...
      public void orientCamera(float partialTicks) {
                  ...
                  if (this.mc.gameSettings.debugCamEnable) {
                        ...
                        for(int i = 0; i < 8; ++i) {
                              ...
                              if (raytraceresult != null) {
                                    ...
                                    if ((!Main.settingsManager.getSettingByName("CameraClip").getValBoolean() || !Main.moduleManager.getModule(NoRender.class).isToggled()) && d7 < d3) {
                                          d3 = d7;
                                    }
                              }
                        }
            ...
            if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
                  if (!this.mc.gameSettings.debugCamEnable) {
                        ...
                        Object object = Reflector.newInstance(Reflector.EntityViewRenderEvent_CameraSetup_Constructor, this, entity, iblockstate1, partialTicks, f6, f7, f1);
                        Reflector.postForgeBusEvent(object);
                        f1 = Reflector.callFloat(object, Reflector.EntityViewRenderEvent_CameraSetup_getRoll);
            ...
            if (this.lightmapUpdateNeeded) {
                  ...
                  if (world != null) {
                        if (Config.isCustomColors()) {
                              ...
                              if (CustomColors.updateLightmap(world, this.torchFlickerX, this.lightmapColors, Minecraft.player.isPotionActive(MobEffects.NIGHT_VISION))) {
                        ...
                        for(int i = 0; i < 256; ++i) {
                              ...
                              if (Minecraft.player.isPotionActive(MobEffects.NIGHT_VISION)) {
                                    Minecraft var10001 = this.mc;
                                    f16 = this.getNightVisionBrightness(Minecraft.player, partialTicks);
                                    f17 = 1.0F / f8;
                                    if (f17 > 1.0F / f9) {
                                          f17 = 1.0F / f9;
                                    }
      ...
      private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
            ...
            int var10006;
            if (flag) {
                  var10002 = (double)partialTicks;
                  var10006 = this.frameCount++;
                  var10005 = this.mc;
                  ShadersRender.setupTerrain(renderglobal, entity, var10002, icamera, var10006, Minecraft.player.isSpectator());
            } else {
                  var10002 = (double)partialTicks;
                  var10006 = this.frameCount++;
                  var10005 = this.mc;
                  renderglobal.setupTerrain(entity, var10002, icamera, var10006, Minecraft.player.isSpectator());
            }
                              ...
                              if (biome.canRain() || biome.getEnableSnow()) {
                                    ...
                                    if (k2 != l2) {
                                          ...
                                          if (world.getBiomeProvider().getTemperatureAtHeight(f1, j2) >= 0.15F) {
                                                ...
                                                d5 = -((double)(this.rendererUpdateCount + l1 * l1 * 3121 + l1 * 45238971 + k1 * k1 * 418711 + k1 * 13761 & 31) + (double)partialTicks) / 32.0D * (3.0D + this.random.nextDouble());
                                                ...
                                                int j4 = i4 >> 16 & '\uffff';
                                                int k4 = i4 & '\uffff';
                                                bufferbuilder.pos((double)l1 - d3 + 0.5D, (double)l2, (double)k1 - d4 + 0.5D).tex(0.0D + d6, (double)k2 * 0.25D + d5 + d7).color(1.0F, 1.0F, 1.0F, f4).lightmap(j4, k4).endVertex();
                                                bufferbuilder.pos((double)l1 + d3 + 0.5D, (double)l2, (double)k1 + d4 + 0.5D).tex(1.0D + d6, (double)k2 * 0.25D + d5 + d7).color(1.0F, 1.0F, 1.0F, f4).lightmap(j4, k4).endVertex();
                                                bufferbuilder.pos((double)l1 + d3 + 0.5D, (double)k2, (double)k1 + d4 + 0.5D).tex(1.0D + d6, (double)l2 * 0.25D + d5 + d7).color(1.0F, 1.0F, 1.0F, f4).lightmap(j4, k4).endVertex();
                                                bufferbuilder.pos((double)l1 - d3 + 0.5D, (double)k2, (double)k1 - d4 + 0.5D).tex(0.0D + d6, (double)l2 * 0.25D + d5 + d7).color(1.0F, 1.0F, 1.0F, f4).lightmap(j4, k4).endVertex();
                                          }
                                    }
                              }
                        }
      ...
      private void updateFogColor(float partialTicks) {
            ...
            EventRenderModel.EventColorFov event = new EventRenderModel.EventColorFov(this, entity, iblockstate1, (double)partialTicks, this.fogColorRed, this.fogColorGreen, this.fogColorBlue);
            event.call();
